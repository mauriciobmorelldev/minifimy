...
export function AccountClient() {
  ...
  useEffect(() => {
    ...
    fetch("/api/account/orders", { headers: { Authorization: `Bearer ${session.token}` } })
      ...
      .catch(() => setStatus("No pudimos cargar tus pedidos desde WooCommerce."))
  ...
  const submitLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setStatus("Validando con WordPress...");
    ...
    setStatus("Listo, sesion validada por WordPress.");
  };

  const submitRegister = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    setStatus("Creando cuenta en WooCommerce...");
    ...
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <section className="rounded-[2rem] bg-[#efe4d0] p-6 shadow-soft md:p-9">
        ...
        <h1 className="mt-5 font-headline text-[2.25rem] font-extrabold leading-tight text-on-surface md:text-5xl">
          Todo se ve en Next, todo se guarda en WooCommerce.
        </h1>
        <p className="mt-4 text-sm leading-7 text-on-surface-variant md:text-base">
          Registro, login y pedidos se consultan contra WordPress/WooCommerce. No duplicamos clientes en Next.
        </p>
      ...
      <section className="rounded-[2rem] bg-white/82 p-5 shadow-soft md:p-8">
        {!session && mode === "login" && (
          <form onSubmit={submitLogin} className="space-y-4">
            <h2 className="font-headline text-2xl font-extrabold text-on-surface">Entrar con WordPress</h2>
        ...
        {!session && mode === "register" && (
          <form onSubmit={submitRegister} className="space-y-4">
            <h2 className="font-headline text-2xl font-extrabold text-on-surface">Crear cuenta WooCommerce</h2>
        ...
        {session && (
          <div>
            <h2 className="font-headline text-2xl font-extrabold text-on-surface">Tus pedidos WooCommerce</h2>
